...
def fefo_batch_select(outlet_id: str, product_id: str, qty_strips_needed: int) -> List[Dict[str, Any]]:
    ...
    batches = list(
        Batch.objects.select_for_update().filter(
            outlet=outlet,
            product=product,
            # Include batches with stock in EITHER field (loose-only batches were
            # previously invisible because qty_strips__gt=0 excluded them entirely)
            expiry_date__gt=today,
            is_active=True,
        ).filter(
            Q(qty_strips__gt=0) | Q(qty_loose__gt=0)
    ...
    # Count both full strips AND loose units (converted to fractional strips)
    # so a batch with qty_strips=0 and qty_loose=8 is still counted as available.
    total_available = sum(
        batch.qty_strips + (batch.qty_loose / (batch.pack_size or 1))
...
# are defined in billing/sale_update_service.py (the canonical location).
# Any import of SaleServiceError should use:
#   from apps.billing.sale_update_service import SaleServiceError
