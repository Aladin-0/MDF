...
class RevisionPermissionsTestCase(BaseRevisionTestCase):
    ...
    def test_readonly_user_cannot_modify(self):
        ...
        payload = {'customerId': str(self.customer.id), 'revisionAction': 'commercial_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'test readonly', 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 3, 'rate': 10}]}
    ...
    def test_manager_actions_on_paid_invoice(self):
        invoice = make_test_invoice(self.outlet, self.billing_staff, self.customer, items=[{'batch': self.batch, 'qty': 5, 'rate': 10}], status='finalized', paid=50)
        ...
        self.assertNotIn('commercial_correction', allowed_actions)

    def test_backend_enforces_permission_regardless_of_frontend(self):
        invoice = make_test_invoice(self.outlet, self.billing_staff, self.customer, items=[{'batch': self.batch, 'qty': 5, 'rate': 10}], status='finalized', paid=50)
        self.authenticate_as(self.billing_staff)
        payload = {'customerId': str(self.customer.id), 'revisionAction': 'paid_bill_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'test enforcement', 'grandTotal': 40, 'cashPaid': 40, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'qtyStrips': 5, 'rate': 10, 'totalAmount': 40}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)