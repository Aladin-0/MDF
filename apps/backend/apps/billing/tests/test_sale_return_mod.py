...
@unittest.skip("Sales Return modification via PUT is not implemented")
class SaleReturnModTests(TestCase):
    ...
    def setUp(self):
        ...
        self.batch = Batch.objects.create(outlet=self.outlet, product=self.product, batch_no='B-001', expiry_date='2026-12-31', mrp=Decimal('10.00'), purchase_rate=Decimal('8.00'), qty_strips=10)
        self.sale = SaleInvoice.objects.create(outlet=self.outlet, customer=self.customer, invoice_no='INV-001', invoice_date='2026-07-01', grand_total=Decimal('100.00'), subtotal=Decimal('100.00'), taxable_amount=Decimal('100.00'), payment_mode='cash', amount_paid=Decimal('100.00'), cash_paid=Decimal('100.00'))
        self.sale_item = SaleItem.objects.create(invoice=self.sale, batch=self.batch, product_name='Dolo 650', pack_size=10, pack_unit='tablet', expiry_date='2026-12-31', qty_strips=10, qty_loose=0, rate=Decimal('10.00'), mrp=Decimal('10.00'), sale_rate=Decimal('10.00'), taxable_amount=Decimal('100.00'), gst_rate=Decimal('0.00'), gst_amount=Decimal('0.00'), total_amount=Decimal('100.00'))
        self.sale_return = SalesReturn.objects.create(outlet=self.outlet, original_sale=self.sale, return_no='RTN-001', return_date='2026-07-02', total_amount=Decimal('50.00'), refund_mode='cash', reason='Wrong Item', created_by=self.admin)
        self.return_item = SalesReturnItem.objects.create(sales_return=self.sale_return, original_sale_item=self.sale_item, batch=self.batch, product_name='Dolo 650', qty_returned=5, return_rate=Decimal('10.00'), total_amount=Decimal('50.00'))
    ...
    def test_sale_return_revise_logs_diff(self):
        ...
        payload = {'outletId': str(self.outlet.id), 'reason': 'Expired Item', 'refundMode': 'cash', 'totalAmount': '100.00', 'revisionReasonCode': 'correction', 'revisionReasonText': 'Wrong quantity', 'items': [{'id': str(self.return_item.id), 'originalSaleItemId': str(self.sale_item.id), 'batchId': str(self.batch.id), 'productName': 'Dolo 650', 'qtyReturned': 10, 'returnRate': '10.00', 'totalAmount': '100.00'}]}
