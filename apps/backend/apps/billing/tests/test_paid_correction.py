...
class PaidCorrectionTestCase(BaseRevisionTestCase):
    ...
    def test_overpayment_creates_customer_advance(self):
        invoice = make_test_invoice(self.outlet, self.billing_staff, self.customer, items=[{'batch': self.batch, 'qty': 10, 'rate': 100}], status='finalized', paid=0)
        ...
        payload = {'customerId': str(self.customer.id), 'revisionAction': 'paid_bill_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'test overpayment', 'grandTotal': 700, 'cashPaid': 1000, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 7, 'rate': 100}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        invoice.refresh_from_db()
        self.assertEqual(invoice.grand_total, 700)
        self.assertEqual(DocumentRevisionV2.objects.filter(object_id=invoice.id).count(), 1)
    ...
    def test_journal_balanced_after_paid_correction(self):
        ...
        make_test_receipt(self.outlet, self.manager, invoice, amount=1000, date_offset_days=0)
        self.authenticate_as(self.manager)
        payload = {'customerId': str(self.customer.id), 'revisionAction': 'paid_bill_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'test balanced journal', 'grandTotal': 700, 'cashPaid': 700, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 7, 'rate': 100}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_billing_staff_cannot_do_paid_correction(self):
        ...
        payload = {'customerId': str(self.customer.id), 'revisionAction': 'paid_bill_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'try paid correction', 'grandTotal': 50, 'cashPaid': 50, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 5, 'rate': 10}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
