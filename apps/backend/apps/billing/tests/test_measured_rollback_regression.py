...
def test_data():
    ...
    # Run seed_ledgers to populate default ledgers
    ...
    # Create Sundry Creditors ledger group if it wasn't seeded or just get it
...
def test_bottle_purchase_and_billing(api_client, test_data):
    ...
    # 1. Purchase Bottle
    purchase_payload = {
        "outletId": str(test_data['outlet'].id),
        "invoiceNo": "PURCH-001",
        "invoiceDate": timezone.now().isoformat(),
        "partyLedgerId": str(test_data['supplier_ledger'].id),
        "subtotal": 100.0,
        "taxableAmount": 100.0,
        "gstAmount": 12.0,
        "grandTotal": 112.0,
        ...
        "items": [
            {
                "masterProductId": str(test_data['product'].id),
                "batchNo": "B-NEW",
                "expiryDate": (date.today() + timedelta(days=300)).isoformat(),
                "mrp": 150.0,
                "purchaseRate": 100.0,
                "saleRate": 150.0,
                "ptr": 100.0,
    ...
    # 2. Product Search in Purchase Context
    ...
    # 3. Product Search in Billing Context
    ...
    sale_payload = {
        "outletId": str(test_data['outlet'].id),
        "invoiceDate": timezone.now().isoformat(),
        "customerId": str(test_data['customer'].id),
        "subtotal": 150.0,
        ...
        "items": [
            {
                "productId": str(test_data['product'].id),
                "batchId": str(batch.id),
                "qtyStrips": 2, # sell 2 bottles
                "qtyLoose": 0,
                "rate": 150.0,
                "mrp": 150.0,
                "saleRate": 150.0,
    ...
    assert batch.qty_strips == 3 # 5 - 2 = 3 bottles left
