...
class SaleModificationTests(TestCase):
    ...
    def setUp(self):
        ...
        self.batch = Batch.objects.create(outlet=self.outlet, product=self.product, batch_no='B001', expiry_date=timezone.now().date() + timedelta(days=365), qty_strips=100, qty_loose=0, mrp=50, purchase_rate=30, pack_size=10, opening_qty=100)
        from apps.inventory.models import StockLedger
        StockLedger.objects.create(outlet=self.outlet, product=self.product, batch=self.batch, txn_type='OPENING', txn_date=timezone.now().date(), qty_in=100, qty_out=0, running_qty=100, running_value=100 * 30)
        self.invoice = SaleInvoice.objects.create(outlet=self.outlet, invoice_no='INV-001', invoice_date=timezone.now(), customer=self.customer, subtotal=Decimal('450.00'), discount_amount=Decimal('0.00'), taxable_amount=Decimal('401.79'), cgst_amount=Decimal('24.11'), sgst_amount=Decimal('24.11'), igst_amount=Decimal('0.00'), cgst=Decimal('6.00'), sgst=Decimal('6.00'), igst=Decimal('0.00'), round_off=Decimal('-0.01'), grand_total=Decimal('450.00'), payment_mode='cash', cash_paid=Decimal('450.00'), upi_paid=Decimal('0.00'), card_paid=Decimal('0.00'), credit_given=Decimal('0.00'), amount_paid=Decimal('450.00'), amount_due=Decimal('0.00'))
        self.item = SaleItem.objects.create(invoice=self.invoice, batch=self.batch, product_name='Paracetamol', pack_size=10, batch_no='B001', expiry_date=self.batch.expiry_date, mrp=50, sale_rate=45, rate=45, qty_strips=10, qty_loose=0, taxable_amount=Decimal('401.79'), gst_amount=Decimal('48.22'), total_amount=Decimal('450.00'), gst_rate=Decimal('12.00'))

    def test_a1_edit_invoice_with_return_blocked(self):
        SalesReturn.objects.create(outlet=self.outlet, return_no='RTN-001', return_date=timezone.now(), original_sale=self.invoice, total_amount=Decimal('45.00'))
    ...
    def test_a2_edit_invoice_with_later_payment_blocked(self):
        receipt = ReceiptEntry.objects.create(outlet=self.outlet, customer=self.customer, date=timezone.now().date(), total_amount=Decimal('100.00'), payment_mode='cash')
        ReceiptAllocation.objects.create(receipt=receipt, invoice=self.invoice, allocated_amount=Decimal('100.00'))
        constraints = get_sale_modification_constraints(self.invoice)
        self.assertFalse(constraints['can_edit'])
    ...
    def test_b1_overpayment_converted_to_advance(self):
        payload = {'grandTotal': 350.0, 'cashPaid': 350.0, 'subtotal': 350.0, 'cashPaid': 450.0, 'upiPaid': 0.0, 'cardPaid': 0.0, 'creditGiven': 0.0, 'items': [{'batchId': str(self.batch.id), 'productId': str(self.product.id), 'qtyStrips': 7, 'qtyLoose': 0, 'rate': 50.0, 'totalAmount': 350.0}], 'customerId': str(self.customer.id)}
        ...
        self.assertEqual(updated_invoice.credit_given, Decimal('-100.00'))
    ...
    def test_c3_discount_stacking_consistency(self):
        payload = {'grandTotal': 299.25, 'cashPaid': 299.25, 'extraDiscountPct': 5.0, 'cashPaid': 299.25, 'items': [{'batchId': str(self.batch.id), 'productId': str(self.product.id), 'qtyStrips': 7, 'qtyLoose': 0, 'rate': 45.0, 'discountPct': 10.0, 'gstRate': 12.0, 'taxableAmount': 267.19, 'gstAmount': 32.06, 'totalAmount': 299.25}], 'customerId': str(self.customer.id)}
        updated_invoice = atomic_sale_update(str(self.invoice.id), payload, str(self.outlet.id), str(self.staff.id))
