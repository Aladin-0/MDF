...
def test_sales_return_partial(default_outlet):
    ...
    invoice = make_test_invoice(default_outlet, staff, status='finalized', paid=100, items=[{'batch': batch, 'qty': 2, 'rate': 50}])
    client = APIClient()
    client.force_authenticate(user=staff.user)
    payload = {'invoice_id': str(invoice.id), 'items': [{'sale_item_id': str(invoice.items.first().id), 'return_qty_strips': 1, 'return_qty_loose': 0}], 'refund_mode': 'cash', 'refund_amount': 50}
...
def test_revise_finalized_bill(default_outlet):
    ...
    client = APIClient()
    client.force_authenticate(user=staff.user)
    payload = {'revision_type': 'correction', 'notes': 'Typo on billing'}
