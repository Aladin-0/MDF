...
class PacketBTests(APITestCase):
    def setUp(self):
        ...
        # We need to make sure we don't have GST ledgers to test the exception
    ...
    def test_missing_gst_ledger_raises_error(self):
        # Create an invoice that requires GST
        ...
        with self.assertRaises(Ledger.DoesNotExist) as cm:
            post_sale_invoice(sale)
            
        self.assertIn("GST ledger", str(cm.exception))
    ...
    def test_select_for_update_called_on_create(self):
        ...
        with patch.object(BatchModel.objects.__class__, 'select_for_update', tracking_sfu):
            try:
                ...
                pass  # We only care that select_for_update was called
    ...
    def test_select_for_update_called_on_modify(self, mock_select_for_update):
        # We need to simulate that select_for_update().get() returns the batch
        # So we mock the select_for_update queryset method
        mock_qs = mock_select_for_update.return_value
        mock_qs.get.return_value = self.batch

        # Create an initial sale to update
        sale = SaleInvoice.objects.create(
            outlet=self.outlet,
            invoice_no="INV-B-UPDATE-001",
            invoice_date=timezone.now(),
            subtotal=Decimal('100.00'),
            taxable_amount=Decimal('100.00'),
            cgst_amount=Decimal('9.00'),
        ...
        with patch('apps.billing.sale_update_service.SaleInvoice.objects.select_for_update') as mock_sale_select:
            ...
            atomic_sale_update(
                ...
                outlet_id=str(self.outlet.id),
                updated_by_id=None
            )
        
        # Verify Batch.objects.select_for_update() was called (during restore old item AND deducting new item)
    ...
    def test_missing_gst_ledger_returns_400_on_modify(self):
        ...
        # Verify transaction is not partially committed (GST amount remains 0)
...
class DeadlockRegressionTests(APITestCase):
    ...
    def setUp(self):
        ...
        # Create two batches with predictable IDs so we can verify sort order
        ...
        StockLedger.objects.create(
            outlet=self.outlet, product=self.product, batch=self.batch_a,
            txn_type="OPENING", txn_date=timezone.now().date(),
            qty_in=50, qty_out=0, rate=Decimal("80.00"),
            running_qty=50, running_value=Decimal("4000.00"),
        )
        StockLedger.objects.create(
            outlet=self.outlet, product=self.product, batch=self.batch_b,
            txn_type="OPENING", txn_date=timezone.now().date(),
            qty_in=50, qty_out=0, rate=Decimal("80.00"),
            running_qty=50, running_value=Decimal("4000.00"),
        )
    ...
    def test_batch_pre_locking_uses_sorted_bulk_query(self):
        ...
        # Deliberate reverse order to verify sort is applied
        items_data = [
            {
                'batchId': batch_b_id,
                'productId': str(self.product.id),
                'qtyStrips': 1, 'qtyLoose': 0, 'rate': '90.00',
                'discountPct': 0, 'gstRate': 0, 'taxableAmount': 90,
                'gstAmount': 0, 'totalAmount': 90,
            },
            {
                'batchId': batch_a_id,
                'productId': str(self.product.id),
        ...
        with patch.object(BatchModel.objects.__class__, 'select_for_update', patched_sfu):
            request_data = {
                'grandTotal': '180.00',
                'cashPaid': '180.00',
                'upiPaid': '0', 'cardPaid': '0', 'creditGiven': '0',
                'paymentMode': 'cash',
            }
            try:
                atomic_sale_save(
                ...
                pass  # We only care about lock order, not full success

        # The sorted() of [batch_b_id, batch_a_id] is [batch_a_id, batch_b_id]
        expected_sorted = sorted([batch_a_id, batch_b_id])
        self.assertEqual(
            sorted(locked_ids_captured),
            expected_sorted,
            "Batch IDs must be locked in sorted order to prevent deadlocks",
        )
    ...
    def test_sale_create_succeeds_with_two_batch_items(self):
        ...
        # Verify stock was deducted from both batches
