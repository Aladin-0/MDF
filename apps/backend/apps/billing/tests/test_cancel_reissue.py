...
class CancelReissueTestCase(BaseRevisionTestCase):
    ...
    def test_cancel_and_reissue_links_invoices(self):
        ...
        payload = {'outletId': str(self.outlet.id), 'customerId': str(self.customer.id), 'revisionAction': 'cancel_and_reissue', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'Wrong customer and items', 'grandTotal': 50, 'cashPaid': 50, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 5, 'rate': 10}]}
        ...
        revision = DocumentRevisionV2.objects.filter(object_id=invoice.id, action='cancel_and_reissue').first()
        self.assertIsNotNone(revision)
        resulting_id = revision.new_snapshot_json.get('_resulting_invoice_id')
        self.assertIsNotNone(resulting_id)
        replacement = SaleInvoice.objects.get(id=resulting_id)
    ...
    def test_cancelled_invoice_cannot_be_modified(self):
        ...
        invoice.save()
        self.authenticate_as(self.manager)
        payload = {'outletId': str(self.outlet.id), 'customerId': str(self.customer.id), 'revisionAction': 'commercial_correction', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'try to modify cancelled', 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 3, 'rate': 10}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
    ...
    def test_cancel_reissue_stock_integrity(self):
        ...
        original_stock = self.batch.qty_strips
        ...
        self.batch.refresh_from_db()
        stock_after_sale = self.batch.qty_strips
        self.authenticate_as(self.manager)
        payload = {'outletId': str(self.outlet.id), 'customerId': str(self.customer.id), 'revisionAction': 'cancel_and_reissue', 'revisionReasonCode': 'CUSTOMER_REQUEST', 'revisionReasonText': 'reissue same items', 'grandTotal': 100, 'cashPaid': 100, 'items': [{'productId': str(self.medicine.id), 'batchId': str(self.batch.id), 'productName': self.medicine.name, 'batchNo': self.batch.batch_no, 'qtyStrips': 10, 'rate': 10}]}
        url = reverse('sale-revise', kwargs={'sale_id': invoice.id})
        response = self.client.post(url, payload, format='json')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.batch.refresh_from_db()
