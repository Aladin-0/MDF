...
from apps.billing.services import UnitIntegrityError
...
def test_walkin_cash_sale(default_outlet, walkin_customer, batch_a):
    ...
    request_data = {'grandTotal': 100.0, 'cashPaid': 100.0, 'paymentMode': 'cash'}
    items_data = [{'productId': str(batch_a.product_id), 'batchId': str(batch_a.id), 'scheduleType': 'OTC', 'qtyStrips': 1, 'qtyLoose': 0, 'mrp': 100, 'saleRate': 100, 'rate': 100, 'totalAmount': 100}]
    invoice = atomic_sale_save(request_data=request_data, outlet=default_outlet, customer=walkin_customer, billed_by=staff, items_data=items_data, schedule_h_data={}, hospital_name='', doctor_id='')
    assert invoice is not None
    assert invoice.amount_due == Decimal('0')
    assert LedgerEntry.objects.filter(outlet=default_outlet, entry_type='receipt', credit=Decimal('100.00')).exists()
...
def test_unit_conversion(default_outlet, walkin_customer, batch_a):
    ...
    request_data = {'grandTotal': 120.0, 'cashPaid': 120.0, 'paymentMode': 'cash'}
    items_data = [{'productId': str(batch_a.product_id), 'batchId': str(batch_a.id), 'qtyStrips': 1, 'qtyLoose': 2, 'rate': 100, 'totalAmount': 120}]
...
def test_discount_application(default_outlet, walkin_customer, batch_a):
    ...
    call_command('seed_ledgers', outlet_id=str(default_outlet.id))
    staff = make_test_staff(default_outlet)
    request_data = {'subtotal': 100.0, 'discountAmount': 10.0, 'grandTotal': 90.0, 'cashPaid': 90.0, 'paymentMode': 'cash'}
    items_data = [{'productId': str(batch_a.product_id), 'batchId': str(batch_a.id), 'qtyStrips': 1, 'qtyLoose': 0, 'rate': 100, 'totalAmount': 100}]
    invoice = atomic_sale_save(request_data=request_data, outlet=default_outlet, customer=walkin_customer, billed_by=staff, items_data=items_data, schedule_h_data={}, hospital_name='', doctor_id='')
...
def test_non_strip_product_qty_loose_rejected(default_outlet, walkin_customer, batch_a):
    ...
    request_data = {'grandTotal': 100.0, 'cashPaid': 100.0, 'paymentMode': 'cash'}
    items_data = [{'productId': str(batch_a.product_id), 'batchId': str(batch_a.id), 'qtyStrips': 1, 'qtyLoose': 5, 'rate': 100, 'totalAmount': 100}]
    with pytest.raises(UnitIntegrityError, match='Loose quantities not permitted for'):
        atomic_sale_save(request_data=request_data, outlet=default_outlet, customer=walkin_customer, billed_by=staff, items_data=items_data, schedule_h_data={}, hospital_name='', doctor_id='')
...
def test_non_strip_product_valid_deduction(default_outlet, walkin_customer, batch_a):
    ...
    request_data = {'grandTotal': 200.0, 'cashPaid': 200.0, 'paymentMode': 'cash'}
    items_data = [{'productId': str(batch_a.product_id), 'batchId': str(batch_a.id), 'qtyStrips': 2, 'qtyLoose': 0, 'rate': 100, 'totalAmount': 200}]
    atomic_sale_save(request_data=request_data, outlet=default_outlet, customer=walkin_customer, billed_by=staff, items_data=items_data, schedule_h_data={}, hospital_name='', doctor_id='')
    batch_a.refresh_from_db()
