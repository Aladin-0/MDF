...
class SaleEditMigrationTests(BaseRevisionTestCase):
    ...
    def get_put_payload(self, invoice):
        ...
        return {'outletId': str(self.outlet.id), 'revisionAction': 'standard_correction', 'revisionReasonCode': 'correction', 'revisionReasonText': 'Test reason', 'grandTotal': float(invoice.grand_total) + 10, 'subtotal': float(invoice.subtotal) + 10, 'cashPaid': float(invoice.grand_total) + 10, 'items': [{'productId': str(item.batch.product_id), 'batchId': str(item.batch_id), 'qtyStrips': item.qty_strips, 'qtyLoose': item.qty_loose, 'rate': float(item.rate) + 1, 'taxableAmount': float(item.taxable_amount) + 10, 'gstAmount': float(item.gst_amount), 'totalAmount': float(item.total_amount) + 10}]}
    ...
    def test_sale_update_rollback_on_business_failure(self):
        url = reverse('sale-detail', kwargs={'sale_id': self.invoice.id})
        payload = self.get_put_payload(self.invoice)
        payload['items'][0]['qtyStrips'] = 9999
        response = self.client.put(url, data=payload, format='json', HTTP_OUTLETID=str(self.outlet.id))
    ...
    def test_history_api_read_path(self):
        ...
        url = reverse('unified-revision-detail', kwargs={'record_type': 'sale', 'record_id': self.invoice.id})
    ...
    def test_history_ui_rendering_parity(self):
        self.test_sale_detail_put_audit_write()
        url = reverse('unified-revision-detail', kwargs={'record_type': 'sale', 'record_id': self.invoice.id}) + f'?outletId={self.outlet.id}'
        response = self.client.get(url, HTTP_OUTLETID=str(self.outlet.id))
    ...
    def test_sale_success_payload_shape_parity(self):
        ...
        response = self.client.put(f'/api/v1/sales/{self.invoice.id}/', payload, format='json', HTTP_OUTLETID=str(self.outlet.id))
        ...
        payload['revisionAction'] = 'cancel_and_reissue'
        response = self.client.post(reverse('sale-revise', kwargs={'sale_id': self.invoice.id}), payload, format='json', HTTP_OUTLETID=str(self.outlet.id))
        self.assertEqual(response.status_code, 200, response.data)
        data = response.data
        self.assertIn('createdAt', data)
        self.assertIn('invoiceNo', data)
