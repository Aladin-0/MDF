...
class TestSaleModificationOptionsView(TestCase):
    ...
    def setUp(self):
        ...
        self.admin = Staff.objects.create(outlet=self.outlet, phone='1111111111', name='Admin', role='admin')
        self.admin.set_password('password')
        self.admin.save()
        self.staff = Staff.objects.create(outlet=self.outlet, phone='2222222222', name='Staff', role='billing_staff')
        self.staff.set_password('password')
        self.staff.save()
        self.invoice = SaleInvoice.objects.create(outlet=self.outlet, invoice_no='INV-100', invoice_date=timezone.now(), subtotal=Decimal('100.00'), taxable_amount=Decimal('100.00'), grand_total=Decimal('100.00'), payment_mode='cash', cash_paid=Decimal('0.00'), amount_paid=Decimal('0.00'), amount_due=Decimal('100.00'))
