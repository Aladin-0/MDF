...
class AuthEventTests(APITestCase):
    ...
    def test_successful_login_creates_log(self):
        ...
        response = self.client.post(self.login_url, {'phone': self.staff.phone, 'password': self.staff_pin})
    ...
    def test_successful_login_captures_ip(self):
        response = self.client.post(self.login_url, {'phone': self.staff.phone, 'password': self.staff_pin}, REMOTE_ADDR='192.168.1.100')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        log = get_latest_log(action='LOGIN')
        self.assertEqual(log.ip_address, '192.168.1.100')

    def test_successful_login_captures_user_agent(self):
        response = self.client.post(self.login_url, {'phone': self.staff.phone, 'password': self.staff_pin}, HTTP_USER_AGENT='Mozilla/5.0 Test Agent')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        log = get_latest_log(action='LOGIN')
    ...
    def test_failed_login_wrong_pin_creates_log(self):
        ...
        response = self.client.post(self.login_url, {'phone': self.staff.phone, 'password': 'wrong'})
    ...
    def test_failed_login_unknown_user_creates_log(self):
        initial_count = count_logs(action='LOGIN_FAILED')
        response = self.client.post(self.login_url, {'phone': '0000000000', 'password': '1234'})
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(count_logs(action='LOGIN_FAILED'), initial_count + 1)
        log = get_latest_log(action='LOGIN_FAILED')
        self.assertIsNotNone(log)
    ...
    def test_failed_login_captures_ip(self):
        response = self.client.post(self.login_url, {'phone': '0000000000', 'password': '1234'}, REMOTE_ADDR='10.0.0.5')
        log = get_latest_log(action='LOGIN_FAILED')
        self.assertEqual(log.ip_address, '10.0.0.5')

    def test_failed_login_user_field_is_null_or_correct(self):
        self.client.post(self.login_url, {'phone': self.staff.phone, 'password': 'wrong'})
        log1 = get_latest_log(action='LOGIN_FAILED')
        self.client.post(self.login_url, {'phone': '0000000000', 'password': '1234'})
    ...
    def test_logout_creates_log(self):
        login_res = self.client.post(self.login_url, {'phone': self.staff.phone, 'password': self.staff_pin})
    ...
    def test_multiple_failed_logins_all_logged(self):
        initial_count = count_logs(action='LOGIN_FAILED')
        for i in range(3):
            self.client.post(self.login_url, {'phone': self.staff.phone, 'password': f'wrong{i}'})
