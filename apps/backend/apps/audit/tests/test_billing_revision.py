...
class BillingRevisionTests(TestCase):
    ...
    def setUp(self):
        ...
        self.invoice = SaleInvoice.objects.create(outlet=self.outlet, customer=self.customer, invoice_no='REV-001', grand_total=Decimal('100.00'), subtotal=Decimal('100.00'), taxable_amount=Decimal('100.00'), amount_due=Decimal('100.00'), amount_paid=Decimal('0.00'), invoice_date=timezone.now().date(), billed_by=self.staff)

    def test_bill_revision_creates_activity_log(self):
        from apps.audit.services import log_activity
        log_activity(action='BILL_REVISION_CREATED', module='billing', entity_type='SaleInvoice', entity_id=self.invoice.id, user=self.staff, description='Fixed amount')
    ...
    def test_modification_blocked_creates_log(self):
        from apps.audit.services import log_activity
        log_activity(action='MODIFICATION_BLOCKED', module='billing', entity_type='SaleInvoice', entity_id=self.invoice.id, user=self.staff, description='Cannot modify paid bill')
    ...
    def test_invoice_cancelled_action_logged(self):
        from apps.audit.services import log_activity
        log_activity(action='INVOICE_CANCELLED', module='billing', entity_type='SaleInvoice', entity_id=self.invoice.id, user=self.staff, description='Bill cancelled')
