...
class AuditAPITests(APITestCase):
    ...
    def setUp(self):
        ...
        ActivityLog.objects.create(user=self.super_admin, outlet=self.outlet, module='inventory', action='CREATE', entity_type='MasterProduct', description='Created product A')
        ActivityLog.objects.create(user=self.billing_staff, outlet=self.outlet, module='billing', action='UPDATE', entity_type='SaleInvoice', description='Updated invoice')
