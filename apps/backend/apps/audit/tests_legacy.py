...
class AuditLogTests(TestCase):
    ...
    def setUp(self):
        ...
        self.user = User.objects.create_user(email='test@example.com', phone='1234567890', password='testpassword', outlet=self.outlet)
        self.admin = User.objects.create_superuser(email='admin@example.com', phone='0987654321', password='adminpassword', outlet=self.outlet)
        self.admin.is_staff = True
        self.admin.is_superuser = True
        self.admin.save()

    def test_activity_log_creation(self):
        log = ActivityLog.objects.create(action='create', module='inventory', user=self.user, entity_type='item', entity_id='1', entity_label='Test Item')
    ...
    def test_log_activity_helper_no_context(self):
        log_activity(action='update', module='billing', user=self.user, description='Test description')
        self.assertEqual(ActivityLog.objects.count(), 1)
    ...
    def test_login_success(self):
        ...
        self.assertEqual(log.user, self.user)
        self.assertEqual(log.ip_is_routable, False)
    ...
    def test_purchase_create_audit_log(self):
        log_activity(action='CREATE', module='purchases', entity_type='PurchaseInvoice', entity_id='12345', entity_label='PI-001', description='Created purchase invoice PI-001 from Dist A for ₹1000', user=self.user, outlet=self.outlet)
    ...
    def test_purchase_update_diff_audit_log(self):
        changes = {'grand_total': {'old': 1000.0, 'new': 1200.0}, 'items_count': {'old': 5, 'new': 6}}
        log_activity(action='UPDATE', module='purchases', entity_type='PurchaseInvoice', entity_id='12345', entity_label='PI-001', description='Updated purchase invoice PI-001', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_billing_create_audit_log(self):
        log_activity(action='CREATE', module='billing', entity_type='SaleInvoice', entity_id='67890', entity_label='SI-001', description='Created sales invoice SI-001 for ₹500', user=self.user, outlet=self.outlet)
    ...
    def test_billing_update_diff_audit_log(self):
        changes = {'discount_amount': {'old': 0.0, 'new': 50.0}, 'grand_total': {'old': 500.0, 'new': 450.0}}
        log_activity(action='UPDATE', module='billing', entity_type='SaleInvoice', entity_id='67890', entity_label='SI-001', description='Updated sales invoice SI-001', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_inventory_adjust_audit_log(self):
        changes = {'qty_strips': {'old': 10, 'new': 5}, 'qty_loose': {'old': 0, 'new': 0}}
        log_activity(action='ADJUST', module='inventory', entity_type='Batch', entity_id='batch-123', entity_label='BATCH001', description='Adjusted stock for batch BATCH001 by -5 (strips). Reason: Damage', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_payment_create_audit_log(self):
        log_activity(action='CREATE', module='payments', entity_type='PaymentEntry', entity_id='pay-123', entity_label='PAY-pay-123', description='Recorded payment of ₹1000 to distributor ABC', user=self.user, outlet=self.outlet)
    ...
    def test_payment_update_diff_audit_log(self):
        changes = {'total_amount': {'old': 1000.0, 'new': 1200.0}, 'payment_mode': {'old': 'cash', 'new': 'upi'}}
        log_activity(action='UPDATE', module='payments', entity_type='PaymentEntry', entity_id='pay-123', entity_label='PAY-pay-123', description='Updated payment entry PAY-pay-123', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_patient_create_audit_log(self):
        log_activity(action='CREATE', module='patient', entity_type='Customer', entity_id='cust-123', entity_label='John Doe', description='Registered new patient John Doe', user=self.user, outlet=self.outlet)
    ...
    def test_patient_update_diff_audit_log(self):
        changes = {'phone': {'old': '9876543210', 'new': '9999999999'}, 'is_chronic': {'old': False, 'new': True}}
        log_activity(action='UPDATE', module='patient', entity_type='Customer', entity_id='cust-123', entity_label='John Doe', description='Updated patient details for John Doe', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_staff_create_audit_log(self):
        log_activity(action='CREATE', module='staff', entity_type='Staff', entity_id='staff-456', entity_label='Jane Smith', description='Created staff account for Jane Smith (admin)', user=self.user, outlet=self.outlet)
        self.assertEqual(ActivityLog.objects.filter(action='CREATE', module='staff').count(), 1)

    def test_staff_role_change_audit_log(self):
        changes = {'role': {'old': 'billing_staff', 'new': 'manager'}, 'can_edit_rate': {'old': False, 'new': True}}
        log_activity(action='UPDATE', module='staff', entity_type='Staff', entity_id='staff-456', entity_label='Jane Smith', description='Updated staff details for Jane Smith', user=self.user, outlet=self.outlet, changes=changes)
    ...
    def test_audit_list_search_filter(self):
        log_activity(action='CREATE', module='patient', entity_type='Customer', entity_id='cust-search-test', entity_label='Unique Searchable Label', description='Created something highly specific keyword123', user=self.user, outlet=self.outlet)
        url = reverse('activity-log-list')
    ...
    def test_audit_export_permission_and_content(self):
        ...
        normal_user = Staff.objects.create_user(phone='8888888888', password='testpassword', name='Normal User', role='billing_staff', outlet=self.outlet, is_active=True)
        from rest_framework_simplejwt.tokens import RefreshToken
    ...
    def test_serializer_contains_ip_is_routable(self):
        ...
        log = ActivityLog.objects.create(action='create', module='inventory', user=self.user, ip_address='10.0.0.1', ip_is_routable=False)
