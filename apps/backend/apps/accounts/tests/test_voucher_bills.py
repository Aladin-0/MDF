...
class VoucherBillModTests(TestCase):
    ...
    def setUp(self):
        ...
        self.invoice = SaleInvoice.objects.create(outlet=self.outlet, customer=self.customer, invoice_no='INV-001', invoice_date='2026-07-01 10:00:00Z', subtotal=Decimal('1000.00'), taxable_amount=Decimal('1000.00'), grand_total=Decimal('1000.00'), payment_mode='credit', amount_paid=Decimal('0.00'), amount_due=Decimal('1000.00'))
        self.voucher = Voucher.objects.create(outlet=self.outlet, voucher_type='receipt', voucher_no='RCT-001', date='2026-07-02', total_amount=Decimal('200.00'), created_by=self.admin)
        ...
        VoucherBillAdjustment.objects.create(voucher=self.voucher, sale_invoice=self.invoice, adjusted_amount=Decimal('200.00'))
    ...
    def test_voucher_modify_bill_allocation(self):
        ...
        payload = {'outletId': str(self.outlet.id), 'voucher_type': 'receipt', 'date': '2026-07-02', 'narration': 'Modified allocation', 'total_amount': '300.00', 'revisionReasonCode': 'correction', 'revisionReasonText': 'Adjusted amount', 'lines': [{'ledger_id': str(self.ledger1.id), 'debit': '300.00', 'credit': '0.00'}, {'ledger_id': str(self.party_ledger.id), 'debit': '0.00', 'credit': '300.00'}], 'bill_adjustments': [{'invoice_id': str(self.invoice.id), 'invoice_type': 'sale', 'adjusted_amount': 300.0}]}
