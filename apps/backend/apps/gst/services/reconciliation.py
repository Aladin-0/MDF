...
def run_advisory_reconciliation(period: str, outlet_id: str, gstin: str):
    ...
    try:
        try:
            ...
            tolerance = Decimal(str(settings.gstr2b_tolerance))
        except OutletSettings.DoesNotExist:
            tolerance = Decimal(str(os.environ.get('GSTR2B_TOLERANCE', '1.0')))
        ...
        purchases_qs = PurchaseInvoice.objects.filter(
            outlet_id=outlet_id,
            invoice_date__year__gte=year-1
        ...
        for p in purchases_qs.iterator(chunk_size=2000):
            ...
            composite_key = f"{gstin}_{norm_inv}"
        ...
        for gstr in gstr2b_records.iterator(chunk_size=2000):
            ...
            composite_key = f"{gstr_supplier}_{norm_gstr_inv}"
            ...
            if not pr:
                ...
                gstr_invoice_value = Decimal(str(gstr.taxable_value)) + Decimal(str(gstr.igst)) + Decimal(str(gstr.cgst)) + Decimal(str(gstr.sgst)) + Decimal(str(gstr.cess))
                diff = abs(gstr_invoice_value - Decimal(str(pr.grand_total)))
                if diff <= tolerance:
                    pr_cgst, pr_sgst, pr_igst = Decimal('0.0'), Decimal('0.0'), Decimal('0.0')
                    if snapshot and snapshot.snapshot_json:
                        pr_cgst = Decimal(str(snapshot.snapshot_json.get('cgst', 0)))
                        pr_sgst = Decimal(str(snapshot.snapshot_json.get('sgst', 0)))
                        pr_igst = Decimal(str(snapshot.snapshot_json.get('igst', 0)))

                    gstr_cgst = Decimal(str(gstr.cgst or 0))
                    gstr_sgst = Decimal(str(gstr.sgst or 0))
                    gstr_igst = Decimal(str(gstr.igst or 0))
