...
class PurchaseEditMigrationTest(TestCase):
    ...
    def setUp(self):
        ...
        self.distributor = Distributor.objects.create(outlet=self.outlet, name='Test Distributor', phone='1234567890', address='123 Test St', city='Test City', state='Test State')
        ...
        self.product = MasterProduct.objects.create(name='Test Medicine', mrp=Decimal('100.00'), pack_size=10)
        self.base_payload = {'outletId': str(self.outlet.id), 'partyLedgerId': str(self.ledger.id), 'invoiceNo': 'TEST-PUR-01', 'invoiceDate': date.today().isoformat() + 'T00:00:00Z', 'dueDate': date.today().isoformat() + 'T00:00:00Z', 'purchaseType': 'credit', 'subtotal': 1000.0, 'discountAmount': 0.0, 'taxableAmount': 1000.0, 'gstAmount': 0.0, 'cessAmount': 0.0, 'grandTotal': 1000.0, 'items': [{'masterProductId': str(self.product.id), 'batchNo': 'BATCH-001', 'expiryDate': '12/26', 'qty': 20, 'actualQty': 20, 'purchaseRate': 50.0, 'mrp': 100.0, 'saleRate': 80.0, 'taxableAmount': 1000.0, 'gstAmount': 0.0, 'totalAmount': 1000.0, 'ptr': 50.0, 'pts': 45.0}]}
    ...
    def test_purchase_edit_happy_path(self):
        ...
        update_payload['subtotal'] = 1200.0
        update_payload['taxableAmount'] = 1200.0
        update_payload['grandTotal'] = 1200.0
        update_payload['items'][0]['qty'] = 24
        update_payload['items'][0]['actualQty'] = 24
        update_payload['items'][0]['taxableAmount'] = 1200.0
        update_payload['items'][0]['totalAmount'] = 1200.0
    ...
    def test_purchase_edit_amount_tax_update(self):
        ...
        update_payload['gstAmount'] = 100.0
        update_payload['grandTotal'] = 1100.0
        update_payload['items'][0]['gstAmount'] = 100.0
        update_payload['items'][0]['totalAmount'] = 1100.0
        updated_invoice = atomic_purchase_update(str(self.invoice.id), update_payload, str(self.outlet.id), str(self.user.id))
    ...
    def test_purchase_edit_line_item_update(self):
        ...
        update_payload['subtotal'] = 1500.0
        update_payload['taxableAmount'] = 1500.0
        update_payload['grandTotal'] = 1500.0
        update_payload['items'][0]['qty'] = 30
        update_payload['items'][0]['actualQty'] = 30
        update_payload['items'][0]['taxableAmount'] = 1500.0
        update_payload['items'][0]['totalAmount'] = 1500.0
        updated_invoice = atomic_purchase_update(str(self.invoice.id), update_payload, str(self.outlet.id), str(self.user.id))
    ...
    def test_purchase_edit_supplier_outstanding_recalculation(self):
        ...
        update_payload['taxableAmount'] = 1200.0
        update_payload['grandTotal'] = 1200.0
        update_payload['items'][0]['purchaseRate'] = 60.0
        update_payload['items'][0]['taxableAmount'] = 1200.0
        update_payload['items'][0]['totalAmount'] = 1200.0
    ...
    def test_purchase_edit_inventory_valuation_consistency(self):
        ...
        update_payload['items'][0]['purchaseRate'] = 55.0
        update_payload['items'][0]['saleRate'] = 85.0
        update_payload['items'][0]['mrp'] = 110.0
        atomic_purchase_update(str(self.invoice.id), update_payload, str(self.outlet.id), str(self.user.id))
        batch = Batch.objects.get(batch_no='BATCH-001', outlet=self.outlet)
    ...
    def test_purchase_edit_atomic_rollback_on_audit_failure(self):
        ...
        update_payload['grandTotal'] = 2000.0
