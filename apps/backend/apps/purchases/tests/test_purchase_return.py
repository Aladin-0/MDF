...
class PurchaseReturnTest(TestCase):
    ...
    def _create_payload(self, qty=10, fail=False):
        return {
            ...
            'subtotal': Decimal('1000.00') if not fail else Decimal('1000.00'),
            'gst_amount': Decimal('120.00'),
            'total_amount': Decimal('1120.00'),
            'items': [
                {
                    'batch_id': str(self.batch.id) if not fail else '00000000-0000-0000-0000-000000000000', # fail deliberately
    ...
    def test_return_creation_accounting(self):
        ...
        # Debits Supplier Ledger, credits Purchase Return ledger
        je = JournalEntry.objects.filter(source_type='DEBIT_NOTE', source_id=note.id).first()
        ...
        # Assuming purchase return ledger gets the subtotal credited (GST goes to other ledgers)
    ...
    def test_stock_reversal(self):
        ...
        # Batch stock is decremented accurately.
        self.batch.refresh_from_db()
        self.assertEqual(self.batch.qty_strips, 15)  # Started at 20, returned 5
    ...
    def test_return_edit_revision(self):
        ...
        self.batch.refresh_from_db()
        # Original: 20. First return: -5 (15). Update to 4: should be 20 - 4 = 16.
        self.assertEqual(self.batch.qty_strips, 16)
        
        note.refresh_from_db()
        self.assertEqual(note.total_amount, Decimal('896.00'))
    ...
    def test_rollback_safety(self):
        ...
        self.batch.refresh_from_db()
        # Rollback Safety: A deliberate error during return saves rolls back all stock and ledger adjustments
