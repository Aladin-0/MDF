...
class PurchaseUnitsTest(TestCase):
    ...
    def _get_base_payload(self, qty=10, qty_loose=0):
        return {'outletId': str(self.outlet.id), 'partyLedgerId': str(self.ledger.id), 'invoiceNo': 'TEST-PUR-UNIT', 'invoiceDate': date.today().isoformat() + 'T00:00:00Z', 'purchaseType': 'credit', 'subtotal': float(qty * 50), 'discountAmount': 0.0, 'taxableAmount': float(qty * 50), 'gstAmount': 0.0, 'cessAmount': 0.0, 'grandTotal': float(qty * 50), 'items': [{'masterProductId': str(self.product.id), 'batchNo': 'BATCH-UNIT', 'expiryDate': '2026-12-01T00:00:00Z', 'qty': qty, 'qtyLoose': qty_loose, 'actualQty': qty * self.product.pack_size + qty_loose, 'purchaseRate': 50.0, 'mrp': 100.0, 'saleRate': 80.0, 'taxableAmount': float(qty * 50), 'gstAmount': 0.0, 'totalAmount': float(qty * 50), 'ptr': 50.0, 'pts': 45.0}]}
