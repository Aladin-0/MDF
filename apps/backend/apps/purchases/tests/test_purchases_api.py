...
def test_purchase_create_api(authenticated_client):
    ...
    # Check that batch was created/updated
...
def test_purchase_update_api(authenticated_client):
    ...
    # First, create a purchase directly via API to ensure correct initial state
    url = reverse('purchase-list-create')
    payload = {
        'outletId': str(outlet.id),
        'distributorId': str(distributor.id),
    ...
    # Update the purchase via PUT
    update_url = reverse('purchase-detail', kwargs={'purchase_id': purchase_id})
    update_payload = {
        'outletId': str(outlet.id),
        'distributorId': str(distributor.id),
        'partyLedgerId': str(distributor_ledger.id),
        'invoiceNo': 'INV-999-REV',
        'invoiceDate': '2026-08-02',
        'revisionReasonCode': 'CORRECTION',
        'revisionReasonText': 'Corrected quantity to 20 due to entry error',
        'subtotal': '200.00',
        'discountAmount': '0',
