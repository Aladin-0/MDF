...
class Command(BaseCommand):
    ...
    def clear_data(self):
        ...
        with transaction.atomic():
            ...
            ledgers.delete()
            self.stdout.write(self.style.SUCCESS(f'Cleared: {sale_count} Sales, {purchase_count} Purchases, {batch_count} Batches, {prod_count} Products, {dist_count} Distributors.'))
    ...
    def setup_base_data(self):
        ...
        sc_group, _ = LedgerGroup.objects.get_or_create(outlet=outlet, name='Sundry Creditors', defaults={'nature': 'liability'})
        party_ledger, _ = Ledger.objects.get_or_create(outlet=outlet, name=f'TEST-DIST-{uuid.uuid4().hex[:6]}', defaults={'group': sc_group, 'opening_balance': 0})
        distributor = Distributor.objects.create(outlet=outlet, name=party_ledger.name, is_active=True)
    ...
    def seed_medicines(self):
        ...
        for sched in schedules:
            for i in range(3):
                med, _ = MasterProduct.objects.get_or_create(name=f'TEST-MED-{sched}-{i}-{uuid.uuid4().hex[:4]}', defaults={'manufacturer': 'Test Pharma', 'category': 'tablet', 'pack_size': 10, 'schedule_type': sched, 'hsn_code': '30049099'})
    ...
    def generate_purchases(self, outlet, staff, distributor, medicines):
        ...
        for i, med in enumerate(medicines):
            ...
            pack_size = 10
            actual_qty = qty * pack_size
            purchase_rate = 50.0
            mrp = 100.0
            taxable = qty * purchase_rate
            gst = taxable * 0.12
            total = taxable + gst
            items_payload.append({'productName': med.name, 'masterProductId': str(med.id), 'batchNo': f'TEST-BATCH-{med.schedule_type}-{i}', 'expiryDate': (date.today() + timedelta(days=700)).isoformat(), 'qty': qty, 'freeQty': 0, 'actualQty': actual_qty, 'pkg': pack_size, 'mrp': mrp, 'purchaseRate': purchase_rate, 'baseLandingRate': purchase_rate, 'saleRate': 80.0, 'ptr': 55.0, 'pts': 50.0, 'discountPct': 0, 'discountAmount': 0, 'gstRate': 12, 'cess': 0, 'taxableAmount': float(taxable), 'gstAmount': float(gst), 'cessAmount': 0, 'totalAmount': float(total)})
        ...
        payload = {'outletId': str(outlet.id), 'invoiceDate': date.today().isoformat(), 'distributorId': str(distributor.id), 'partyLedgerId': str(distributor.linked_ledger.id) if hasattr(distributor, 'linked_ledger') else None, 'invoiceNo': invoice_no, 'items': items_payload, 'subtotal': subtotal, 'taxableAmount': subtotal, 'discountAmount': 0, 'roundOff': 0, 'cessAmount': 0, 'gstAmount': gst_amount, 'grandTotal': grand_total, 'paymentStatus': 'unpaid', 'purchaseType': 'credit'}
        ...
        self.stdout.write(self.style.SUCCESS(f'  -> DB Verified: Purchase {invoice.invoice_no} created with {item_count} items. DB Subtotal matches: {db_subtotal}'))
    ...
    def generate_sales(self, outlet, staff, medicines):
        ...
        created_invoices = []
        customer, _ = Customer.objects.get_or_create(outlet=outlet, phone='9999999999', defaults={'name': 'Test Sale Customer'})
        batches = Batch.objects.filter(batch_no__startswith='TEST-BATCH-')
        ...
        for batch in batches:
            ...
            payload = {'outletId': str(outlet.id), 'customerId': str(customer.id), 'items': [{'batchId': str(batch.id), 'productId': str(batch.product.id) if batch.product else '', 'name': batch.product.name if batch.product else 'Unknown', 'qtyStrips': sell_qty, 'qtyLoose': 0, 'rate': float(batch.mrp), 'discountPct': 0, 'discountAmount': 0, 'taxableAmount': taxable_amount, 'gstAmount': gst_amount, 'cgstAmount': gst_amount / 2, 'sgstAmount': gst_amount / 2, 'igstAmount': 0, 'gstRate': 12, 'totalAmount': taxable_amount + gst_amount}], 'subtotal': taxable_amount, 'discountAmount': 0, 'taxableAmount': taxable_amount, 'gstAmount': gst_amount, 'cgstAmount': gst_amount / 2, 'sgstAmount': gst_amount / 2, 'igstAmount': 0, 'cessAmount': 0, 'roundOff': 0, 'grandTotal': taxable_amount + gst_amount, 'paymentMode': 'cash', 'cashPaid': taxable_amount + gst_amount, 'upiPaid': 0, 'cardPaid': 0, 'creditGiven': 0}
            med_schedule = batch.product.schedule_type if batch.product else ''
            if med_schedule in ['H', 'H1', 'X']:
                payload['scheduleHData'] = {'patientName': 'Strict Patient', 'patientAge': 30, 'patientAddress': 'Test Patient Address', 'doctorName': 'Dr. Strict', 'doctorAddress': 'Test Hospital', 'doctorRegNo': 'REG-12345', 'prescriptionNo': f'RX-{uuid.uuid4().hex[:4].upper()}'}
            ...
            self.stdout.write(self.style.SUCCESS(f'  -> DB Verified: Sale {sale.invoice_no} created with {sale.items.count()} items.'))
