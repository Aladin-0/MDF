...
def test_stock_in_purchase(inventory_outlet, strip_product, strip_batch):
    ...
    payload = {
        'outletId': str(inventory_outlet.id),
        'partyLedgerId': str(ledger.id),
        'invoiceNo': 'TEST-PUR-1',
        'invoiceDate': timezone.now().date().isoformat() + 'T00:00:00Z',
        'purchaseType': 'credit',
        'subtotal': 500.0,
        'discountAmount': 0.0,
        'taxableAmount': 500.0,
        ...
        'items': [
            {
                'masterProductId': str(strip_product.id),
                'batchNo': strip_batch.batch_no,
                'expiryDate': strip_batch.expiry_date.isoformat() + 'T00:00:00Z',
                'qty': 10,
                'actualQty': 10,
                'purchaseRate': 50.00,
                'mrp': 100.00,
...
def test_stock_out_sale(inventory_outlet, strip_product, strip_batch, box_product, box_batch):
    ...
    # Strip: 1 strip and 2 loose deduction from pack_size 10
    # Equivalent to 12 loose deduction
    assert strip_batch.qty_strips == initial_strip_strips - 2
    assert strip_batch.qty_loose == 8

    # Box: 2 strips deduction (box is effectively a strip conceptually for API/stock)
...
def test_returns(inventory_outlet, strip_product, strip_batch):
    ...
    # Needs a purchase accounts group for Purchase Returns ledger
    ...
    # Sale to return from
    request_data = {
        'grandTotal': '100.00', 'subtotal': '100.00', 'cashPaid': '100.00',
        'creditGiven': '0', 'upiPaid': '0', 'cardPaid': '0',
        'invoiceDate': timezone.now().isoformat(), 'extraDiscountPct': '0'
    }
    items_data = [{
        'productId': str(strip_product.id),
        'batchId': str(strip_batch.id), 'qtyStrips': 5, 'qtyLoose': 0,
        'rate': 20.00, 'taxableAmount': 100.00, 'gstAmount': 0.0
    }]
    
    initial_strips = strip_batch.qty_strips
    
    sale = atomic_sale_save(
        request_data=request_data, outlet=inventory_outlet, customer=customer,
        billed_by=staff, items_data=items_data, schedule_h_data={},
    ...
    return_payload = {
        ...
        'items': [{
            ...
            'batchId': str(strip_batch.id),
            'qtyReturned': 20, # 2 strips * 10 tablets/strip
            'returnRate': 20.00
        }]
    }
    create_sales_return(return_payload, str(inventory_outlet.id), str(staff.id))
    strip_batch.refresh_from_db()
    assert strip_batch.qty_strips == initial_strips - 3 # Restored 2
    ...
    dn_payload = {
        ...
        'items': [
            {
                'batch_id': str(strip_batch.id),
                'product_name': strip_product.name,
                'qty': Decimal('1'),
                'rate': Decimal('100.00'),
    ...
    strip_batch.refresh_from_db()
    assert strip_batch.qty_strips == initial_strips - 4 # Decreased 1
