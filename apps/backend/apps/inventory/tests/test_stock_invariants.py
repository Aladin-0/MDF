...
def test_stock_invariant_exact_deduction():
    ...
    invoice = atomic_sale_save(request_data=request_data, outlet=outlet, customer=None, billed_by=staff, items_data=items_data, schedule_h_data=None, hospital_name='', doctor_id=None)
...
def test_stock_invariant_negative_stock_guard():
    ...
    request_data = {'grandTotal': 300.0, 'subtotal': 300.0, 'discountAmount': 0, 'cashPaid': 300.0, 'paymentMode': 'cash', 'invoiceDate': timezone.now().isoformat()}
    items_data = [{'productId': str(product.id), 'batchId': str(batch.id), 'qtyStrips': 3, 'qtyLoose': 0, 'rate': '100.00', 'gstRate': '0'}]
...
def test_stock_invariant_deterministic_rebuild():
    ...
    post_stock_ledger_entry(outlet=outlet, product=product, batch=batch, txn_type='PURCHASE_IN', txn_date=date(2025, 1, 1), voucher_type='Purchase Invoice', voucher_number='PINV-001', party_name='Supplier', qty_in=10, qty_out=0, rate=80.0)
    request_data = {'grandTotal': 200.0, 'subtotal': 200.0, 'discountAmount': 0, 'cashPaid': 200.0, 'paymentMode': 'cash', 'invoiceDate': date(2025, 1, 2).isoformat()}
    items_data = [{'productId': str(product.id), 'batchId': str(batch.id), 'qtyStrips': 2, 'qtyLoose': 0, 'rate': '100.00', 'gstRate': '0'}]
    atomic_sale_save(request_data=request_data, outlet=outlet, customer=None, billed_by=staff, items_data=items_data, schedule_h_data=None, hospital_name='', doctor_id=None)
    batch.refresh_from_db()
