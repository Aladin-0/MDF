...
def test_no_negative_stock(api_client, strip_batch):
    ...
    payload = {'outletId': str(strip_batch.outlet.id), 'batchId': str(strip_batch.id), 'adjustType': 'sub', 'adjustUnit': 'strips', 'qty': '-200', 'reason': 'Damaged'}
...
def test_unit_integrity_enforcement(api_client, box_batch):
    ...
    payload = {'outletId': str(box_batch.outlet.id), 'customerId': str(customer.id), 'grandTotal': '250.00', 'subtotal': '250.00', 'discountAmount': '0', 'paymentMode': 'cash', 'cashPaid': '250.00', 'invoiceDate': '2026-08-01', 'items': [{'productId': str(box_batch.product.id), 'batchId': str(box_batch.id), 'qtyStrips': 0, 'qtyLoose': 1, 'saleRate': '250.00', 'mrp': '250.00'}]}
    response = api_client.post(url, payload, format='json')
    assert response.status_code == status.HTTP_400_BAD_REQUEST
...
def test_atomicity_on_failure(api_client, strip_batch):
    ...
    url = reverse('sale-list-create')
    payload = {'outletId': str(strip_batch.outlet.id), 'customerId': str(customer.id), 'grandTotal': '50.00', 'subtotal': '50.00', 'discountAmount': '0', 'paymentMode': 'cash', 'cashPaid': '50.00', 'invoiceDate': '2026-08-01', 'items': [{'productId': str(strip_batch.product.id), 'batchId': str(strip_batch.id), 'qtyStrips': 1, 'qtyLoose': 0, 'saleRate': '50.00', 'mrp': '50.00'}]}
