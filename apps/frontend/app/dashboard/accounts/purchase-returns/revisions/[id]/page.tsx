...
export default function RevisionDetailPage({ params }: { params: { id: string } }) {
    ...
    useEffect(() => {
        if (!outlet) return;
        api.get(`/audit/revisions/debit_note/${returnId}/`, { params: { outletId: outlet.id } })
            .then(res => {
                setData(res.data);
                setLoading(false);
            })
    ...
    const { record: debit_note, revisions } = data;
            ...
            <div className="flex justify-between items-center bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                <div>
                    <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                        ...
                        Purchase Return History: {debit_note.debitNoteNo}
                    </h1>
                    <div className="flex gap-4 mt-2 text-sm text-gray-500">
                        <span className="flex items-center gap-1"><Clock className="h-4 w-4" /> Created: {format(new Date(debit_note.createdAt), 'dd MMM yyyy, hh:mm a')}</span>
                        <span className="flex items-center gap-1"><User className="h-4 w-4" /> By: {debit_note.createdBy}</span>
                    </div>
                </div>
            </div>
                                            ...
                                            <div className="bg-amber-50/50 border border-amber-100 rounded-md overflow-hidden divide-y divide-amber-100">
                                                {rev.diff_summary_json.items_modified.map((mod: any, i: number) => (
                                                    <div key={i} className="p-3 text-sm">
                                                        <div className="font-medium text-amber-900 mb-2">{mod.product_name || 'Unknown Product'}</div>
                                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
                                                            {Object.entries(mod).map(([field, vals]: [string, any]) => {
                                                                if (field === 'id' || field === 'product_name') return null;
                                                                return (
                                            ...
                                            <div className="bg-green-50/50 border border-green-100 rounded-md overflow-hidden divide-y divide-green-100">
                                                {rev.diff_summary_json.items_added.map((item: any, i: number) => (
                                                    <div key={i} className="p-3 text-sm flex justify-between items-center">
                                                        <div className="font-medium text-green-900">{item.product_name || 'Unknown Product'}</div>
                                        ...
                                        <div className="space-y-2 mt-4">
                                            ...
                                            <div className="bg-red-50/50 border border-red-100 rounded-md overflow-hidden divide-y divide-red-100">
                                                {rev.diff_summary_json.items_removed.map((item: any, i: number) => (
                                                    <div key={i} className="p-3 text-sm flex justify-between items-center">
                                                        <div className="font-medium text-red-900 line-through opacity-70">{item.product_name || 'Unknown Product'}</div>
                                                        <div className="text-xs text-red-700 opacity-70">
                                                            Qty: {item.qty} | Rate: ₹{item.rate}
                                                        </div>
                                                    </div>
                                                ))}
