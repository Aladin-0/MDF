...
test.describe('Sale Return Modification Tracking', () => {
  ...
  test.beforeEach(async ({ api }) => {
    ...
    try {
        const response = await api.createSaleReturn(outletId, 'dummy-sale-id');
  ...
  test.describe('Category A: Core CRUD & Tracking', () => {
    test('Create a new record — assert no false modified history', async ({ api }) => {
      ...
      const revResponse = await api.apiRequest('GET', `/audit/revisions/sales/returns/${returnId}/`);
