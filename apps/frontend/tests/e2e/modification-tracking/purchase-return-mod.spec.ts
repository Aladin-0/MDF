...
test.describe('Purchase Return Modification Tracking', () => {
  ...
  test.beforeEach(async ({ api }) => {
    ...
    try {
        const response = await api.createPurchaseReturn(outletId, 'dummy-purchase-id');
  ...
  test.describe('Category A: Core CRUD & Tracking', () => {
    test('Create a new record — assert no false modified history', async ({ api }) => {
      if (returnId === 'dummy-id') return test.skip();
      const revResponse = await api.apiRequest('GET', `/audit/revisions/purchases/returns/${returnId}/`);
